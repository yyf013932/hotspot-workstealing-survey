...
public:
    ...
    void do_oop_work(T *p) {
        ...
        if (PSScavenge::should_scavenge(p, _to_space)) {
            PSScavenge::copy_and_push_safe_barrier<T, /*promote_immediately=*/false>(
                    _promotion_manager, p);
        }
    }
...
bool PSScavenge::invoke_no_policy() {
    ...
    TimeStamp scavenge_midpoint;//marking finish
    TimeStamp scavenge_exit;

    TimeStamp init_phase_exit;
...

    {
        ...
        PSPromotionManager *promotion_manager = PSPromotionManager::vm_thread_promotion_manager();
        {
            ...
            init_phase_exit.update();

            gc_task_manager()->execute_and_wait(q);

            StealTask::_gc_count++;

            uint gc_count = StealTask::_gc_count;//the total count of gc

            tty->print_cr(
                    "steal : [ total_attempts , fail_attempts , fail_ratio]\n\t[ %d , %d , %f]\n",
                    StealTask::_steal_attempts / gc_count, StealTask::_steal_fail / gc_count,
                    (StealTask::_steal_fail + 0.0) / (StealTask::_steal_attempts));

            tty->print_cr("ave steal per steal task(per gc): %d",
                          StealTask::_steal_attempts / (ParallelGCThreads * gc_count));

            tty->print_cr(
                    "after first steal fail : [ total_attempts , fail_attempts , fail_ratio]\n"
                    "\t[ %d , %d , %f]\n",
                    StealTask::_after_first_fail_attempts / gc_count,
                    StealTask::_after_first_fail_attempts_fail / gc_count,
                    (StealTask::_after_first_fail_attempts_fail + 0.0) / StealTask::_after_first_fail_attempts);
        }
            ...
            if (UseAdaptiveSizePolicy) {
                ...
                if (PrintTenuringDistribution) {
                    ...
                    gclog_or_tty->print_cr(
                            "Desired survivor size " SIZE_FORMAT " bytes, new threshold %u (max %u)",
    ...
    if (PrintGCTaskTimeStamps) {
        tty->print_cr("VM-Thread " INT64_FORMAT " " INT64_FORMAT " " INT64_FORMAT,
                      ...
                      scavenge_exit.ticks());
        jlong init_time = init_phase_exit.ticks() - scavenge_entry.ticks();
    ...
    if (PrintGCDetails && Verbose) {
        ...
        if (young_gen->used_in_bytes() <
            ...
            gclog_or_tty->print_cr(" padded_promoted_average is greater"
                                   " than maximum promotion = " SIZE_FORMAT,
...
void PSScavenge::initialize() {
    ...
    _ref_processor =
            new ReferenceProcessor(mr,                         // span
                                   ...
                                   (ParallelGCThreads > 1), // mt processing
