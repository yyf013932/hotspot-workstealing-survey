...
void GCTaskThread::print_task_time_stamps() {
    ...
    tty->print_cr("cost time:[root task,steal_task,other]\n\t"
                  "[" INT64_FORMAT " " INT64_FORMAT " " INT64_FORMAT "]\n",
                  root_task_time, steal_task_time, other_task_time);
        ...
        for (; /* break */;) {
            ...
            if (!is_idle_task) {
                ...
                if (PrintGCTaskTimeStamps) {
                    ...
                    jlong cost_time = (time_stamp->exit_time() - time_stamp->entry_time());
                    //ordinary_task,maybe root_task or steal_task
                    if (task->is_ordinary_task()) {
                        if (strcmp(task->name(), "steal-task")) {//is steal task
                            steal_task_time += cost_time;
                        } else {//other is root task
                            root_task_time += cost_time;
                        }
                    } else {
                        other_task_time += cost_time;
                        //noop task maybe
                    }
                }
            } else {
